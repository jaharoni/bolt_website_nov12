...
async function main() {
  ...
  const backupFile = path.join(process.cwd(), 'backups', backupTimestamp, 'supabase-backup.json');
  ...
  if (!confirmRestore) {
    ...
    console.error('To proceed, run with: CONFIRM_RESTORE=yes npm run db:restore <timestamp>');
    process.exit(1);
  }

  for (const [tableName, data] of Object.entries(backupData.tables)) {
    await restoreTable(tableName, data as any[]);
  }
