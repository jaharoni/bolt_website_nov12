...
DROP POLICY IF EXISTS "Anyone can view essay media" ON essays_media;
CREATE POLICY "Anyone can view essay media"
  ON essays_media FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Authenticated users can manage essay media" ON essays_media;
CREATE POLICY "Authenticated users can manage essay media"
  ON essays_media FOR ALL
  USING (auth.role() = 'authenticated');

-- RLS Policies for galleries
DROP POLICY IF EXISTS "Anyone can view active galleries" ON galleries;
...
DROP POLICY IF EXISTS "Anyone can view site zones" ON site_zones;
CREATE POLICY "Anyone can view site zones"
  ON site_zones FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Authenticated users can manage zones" ON site_zones;
CREATE POLICY "Authenticated users can manage zones"
  ON site_zones FOR ALL
  USING (auth.role() = 'authenticated');

-- RLS Policies for text_blocks
DROP POLICY IF EXISTS "Anyone can view text blocks" ON text_blocks;
CREATE POLICY "Anyone can view text blocks"
  ON text_blocks FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Authenticated users can manage text blocks" ON text_blocks;
