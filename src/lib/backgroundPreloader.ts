...
import { Media, SiteZone } from "./types";
...
const runWhenIdle = (callback: () => void, timeout = DEFAULT_IDLE_TIMEOUT) => {
  if (
...
class BackgroundPreloader {
  ...
  private zoneConfigCache: Map<string, BackgroundZoneConfig> = new Map();
  ...
  async preloadForPage(
    ...
    if (
      ...
      console.debug(`[backgroundPreloader] using cached ${pageKey}`);
      return cached.media;
    }
  ...
  private async fetchMediaForPage(
    ...
    const zoneKey =
      pageKey === "home" ? "home.background" : `page.${pageKey}.background`;
    ...
    if (!zoneConfig || forceRefresh) {
      ...
      if (zone) {
        const typedZone = zone as SiteZone;
        const config =
          (typedZone.config_json as BackgroundZoneConfig | null) ?? {};

        zoneConfig = {
          randomization_enabled:
            typedZone.randomization_enabled ??
            config.randomization_enabled ??
            true,
          static_media_id:
            typedZone.static_media_id ?? config.static_media_id ?? null,
    ...
    if (
      zoneConfig?.randomization_enabled === false &&
      zoneConfig?.static_media_id
    ) {
    ...
    if (zoneConfig?.source) {
      ...
      if (cfg.type === "gallery") {
        ...
        if (!mediaList || forceRefresh) {
          ...
          mediaList = (data ?? [])
            .map((record) => record.media as Media)
            .filter(Boolean);
  ...
  private async recordBackgroundView(
    pageKey: string,
    mediaId: string,
  ): Promise<void> {
    try {
  ...
  prefetchAllPages(): void {
    ...
    runWhenIdle(() => {
      void this.preloadMultiple(allPages);
    });
  }
}
