...
export async function refresh(): Promise<BGConfig>{
  try{
    ...
    mem = { pages:{}, fallback:{ mode:"random", images:[], folders:["media"] } };
    return mem;
  }
}
...
function normalizeCfg(cfg: BGConfig): BGConfig{
  const out: BGConfig = { pages:{}, fallback: cfg.fallback || { mode:"random", images:[], folders:["media"] } };
  const fixUrl = (u:string)=> u.startsWith("/")? u : ("/" + u.replace(/^\/+/,""));
